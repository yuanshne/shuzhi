# -*- coding: utf-8 -*-
# 内联 src/engine.js / src/art.js / src/puzzle-client.js → 生成根目录 index.html（各文件包裹 IIFE 隔离作用域）
import io, os
...
cli = io.open(os.path.join(src, 'puzzle-client.js'), encoding='utf-8').read()
tpl = io.open(os.path.join(src, 'template.html'), encoding='utf-8').read()
assert '/*__ENGINE__*/' in tpl and '/*__ART__*/' in tpl and '/*__CLIENT__*/' in tpl
NL = chr(10)
wrap = lambda code: '(function(){' + NL + code + NL + '})();'
out = (tpl.replace('/*__ENGINE__*/', wrap(eng))
          .replace('/*__ART__*/', wrap(art))
          .replace('/*__CLIENT__*/', wrap(cli)))
